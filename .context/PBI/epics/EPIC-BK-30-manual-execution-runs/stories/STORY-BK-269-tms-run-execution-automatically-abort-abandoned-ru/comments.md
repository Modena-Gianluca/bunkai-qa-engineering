...
SELECT id FROM runs WHERE status = 'running' AND last*step*activity_at < NOW() - interval '{threshold} hours'
...
- run*atcs rows: status computed from their child run*steps (same logic as manual abort)
- run_steps rows: pending steps are set to "skipped" status
- runs table: status becomes "aborted", finish_time is set, reason is set to the system-generated template
- progress_pct: recomputed to reflect the final state
