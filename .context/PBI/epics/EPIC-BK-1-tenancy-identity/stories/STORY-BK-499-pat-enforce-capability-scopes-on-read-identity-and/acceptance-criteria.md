...
## Refined Acceptance Criteria (Finalized 2026-08-21 — see AI Product Owner & AI Tech Lead Ruling comment; AC1 amended 2026-08-25 — see Stage 2 Finding B ruling comment)

### AC1 — Bootstrap workspace (carried verbatim from BK-262; Scenario 2 amended 2026-08-25)
...
Scenario: A zero-scope token cannot be minted
  Given Karim is authenticated via browser session (token issuance is cookie-only)
  When Karim requests POST /api/v1/tokens with an empty scopes array
  Then the request is rejected with 422 (validation error, not a 403 authorization error)
...
***Full analysis, rationale per ruling, and test outlines******:****** see the Acceptance Test Plan (ATP) field and the "AI Product Owner & AI Tech Lead Ruling" comments (2026-08-21 finalization; 2026-08-25 AC1 Scenario 2 correction).***
