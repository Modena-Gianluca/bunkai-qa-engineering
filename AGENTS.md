...
16. **XRAY GUARD — QUALITY OVER SPEED**: Quality over speed — 3/5 min task may take 10/15 min if needed to respect the full flow. Before ANY `bun xray` or `acli` write (Test/ATS/ATP/ATR/link), MUST load owning skill (`/sprint-testing` for ATS Set-first, `/test-documentation` for TC/ATP/ATR, `/xray-cli` + `/acli` for HOW) and read `agentic-qa-core/references/traceability-linking.md:23` (ATS mandatory) + verify nomenclature `BK-XXX: TC01: should ...` against live `BK-256`. Enforced locally by `bun run xray:guard` and `.husky/pre-commit` (blocks commit on `T1` or missing `ATS`).
